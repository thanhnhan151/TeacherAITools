...
namespace TeacherAITools.Application.Comments.Commands.UpdateComment
{
    public class UpdateCommentCommandHandler(
        IUnitOfWork unitOfWork) : IRequestHandler<UpdateCommentCommand, Response<GetCommentResponse>>
    {
        private readonly IUnitOfWork _unitOfWork = unitOfWork;

        public async Task<Response<GetCommentResponse>> Handle(UpdateCommentCommand request, CancellationToken cancellationToken)
        {
            ...
            comment.CommentBody = request.Comment.Body;

            await _unitOfWork.Comments.UpdateAsync(comment);
            await _unitOfWork.CompleteAsync();

            return new Response<GetCommentResponse>(code: (int)ResponseCode.UPDATED_SUCCESS, message: ResponseCode.UPDATED_SUCCESS.GetDescription());
        }
    }
}
